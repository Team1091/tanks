...
public class BestTankEver implements AI {
    ...
    public Control act(@NotNull Sensor sensor, @NotNull Tank tank) {
        ...
        var closestPickup = sensor.getPickups().stream().min(Comparator.comparingDouble((it) -> it.getPos().distanceTo(tank.getPos())));
        ...
        var enemy = sensor.getTargets().stream().min(Comparator.comparingDouble((it) -> it.getPos().distanceTo(tank.getPos())));
